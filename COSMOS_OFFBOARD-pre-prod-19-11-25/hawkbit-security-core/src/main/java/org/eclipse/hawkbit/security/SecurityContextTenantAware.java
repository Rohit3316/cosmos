...
public class SecurityContextTenantAware implements TenantAware {
    ...
    private static SecurityContext buildTenantAsUserSecurityContext(final String tenant) {
        final String username = SecurityContextHolder.getContext().getAuthentication().getName();
        final Collection<? extends GrantedAuthority> authorities = SecurityContextHolder.getContext()
                .getAuthentication().getAuthorities();
        return buildUserSecurityContext(tenant, username, authorities);
    }
    ...
    /**
     * Method to get the current authenticated tenant from TenantAwareAuthenticationDetails/UserPrincipal
    ...
    public String getCurrentTenant() {
        ...
        if (context.getAuthentication() != null) {
            ...
            if (context.getAuthentication().getDetails() instanceof final TenantAwareAuthenticationDetails tenantAwareAuthenticationDetails) {
                return tenantAwareAuthenticationDetails.getTenant();
            } else if (principal instanceof final UserPrincipal userPrincipal) {
                return userPrincipal.getTenant();
            }
        }
        return null;
    }
    ...
    /**
     * Method to get the current authenticated username from OidcUser/UserPrincipal
    ...
    public String getCurrentUsername() {
        ...
        if (authentication != null && authentication.getPrincipal() != null) {
            if (authentication.getPrincipal() instanceof final OidcUser user) {
                return user.getPreferredUsername();
            } else if (authentication.getPrincipal() instanceof final UserPrincipal principal) {
    ...
    @Override
    public <T> T runTenantAsLoggedUser(final String tenant, final TenantRunner<T> tenantRunner) {
        return runInContext(buildTenantAsUserSecurityContext(tenant), tenantRunner);
    }
