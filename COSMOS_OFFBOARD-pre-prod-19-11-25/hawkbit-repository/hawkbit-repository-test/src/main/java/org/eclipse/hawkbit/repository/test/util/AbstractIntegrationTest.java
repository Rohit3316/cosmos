...
@SpringBootTest
@ContextConfiguration(classes = {TestConfiguration.class, TestChannelBinderConfiguration.class})
...
public abstract class AbstractIntegrationTest {
    ...
    protected static final String DEFAULT_RSP_KEY = "hawkbit.server.tenant.configuration.rollout-mandatory-support-package-rsp.defaultValue";
    protected static final String DEFAULT_ESP_KEY = "hawkbit.server.tenant.configuration.rollout-mandatory-support-package-esp.defaultValue";
    ...
    @Autowired
    protected SoftwareModuleFormatManagement softwareModuleFormatManagement;

    @Autowired
    protected SoftwareInstallerTypeManagement softwareInstallerTypeManagement;
    ...
    @Autowired
    protected DeploymentLogManagement deploymentLogManagement;
    ...
    protected SoftwareModuleFormat format;

    protected SoftwareInstallerType swInstallerType;
    ...
    static void dynamicProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("spring.datasource.url",
                () -> "jdbc:h2:mem:" + uniqueDbName + ";DB_CLOSE_DELAY=-1;MODE=LEGACY");
    ...
    protected DistributionSetAssignmentResult assignDistributionSet(final long dsID, final String controllerId) {
        return assignDistributionSet(dsID, controllerId, MgmtRolloutUserAcceptanceRequired.NO);
    }

    public static int findFreePort() {
        try (ServerSocket serverSocket = new ServerSocket(0)) {
            serverSocket.setReuseAddress(true);
            return serverSocket.getLocalPort(); // Returns the port that was assigned.
        } catch (IOException e) {
    ...
    protected DistributionSetAssignmentResult assignDistributionSet(final long dsID, final String controllerId,
        ...
        return assignDistributionSet(dsID, Collections.singletonList(controllerId), userAcceptanceRequired);
    }

    protected DistributionSetAssignmentResult assignDistributionSet(final long dsID, final String controllerId,
                                                                    final MgmtRolloutUserAcceptanceRequired userAcceptanceRequired, final long forcedTime) {
        return assignDistributionSet(dsID, Collections.singletonList(controllerId), userAcceptanceRequired, forcedTime);
    }

    protected DistributionSetAssignmentResult assignDistributionSet(final long dsID, final List<String> controllerIds,
                                                                    final MgmtRolloutUserAcceptanceRequired userAcceptanceRequired) {
        return assignDistributionSet(dsID, controllerIds, userAcceptanceRequired, RepositoryModelConstants.NO_FORCE_TIME);
    }

    protected DistributionSetAssignmentResult assignDistributionSet(final long dsID, final List<String> controllerIds,
                                                                    final MgmtRolloutUserAcceptanceRequired userAcceptanceRequired, final long forcedTime) {
        return assignDistributionSet(dsID, controllerIds, userAcceptanceRequired, forcedTime, null);
    }

    protected DistributionSetAssignmentResult assignDistributionSet(final long dsID, final List<String> controllerIds,
                                                                    final MgmtRolloutUserAcceptanceRequired userAcceptanceRequired, final long forcedTime, final Integer weight) {
        ...
        final List<DeploymentRequest> deploymentRequests = controllerIds.stream()
                .map(id -> DeploymentManagement.deploymentRequest(id, dsID).setUserAcceptanceRequired(userAcceptanceRequired)
    ...
    protected DistributionSetAssignmentResult assignDistributionSet(final DistributionSet ds,
        ...
        return assignDistributionSet(ds.getId(), targetIds, MgmtRolloutUserAcceptanceRequired.NO);
    }

    protected DistributionSetAssignmentResult assignDistributionSet(final Long dsId, final List<String> targetIds,
                                                                    final int weight) {
        return assignDistributionSet(dsId, targetIds, MgmtRolloutUserAcceptanceRequired.NO, RepositoryModelConstants.NO_FORCE_TIME,
                weight);
    }
    ...
    protected DistributionSetAssignmentResult assignDistributionSetWithMaintenanceWindow(final long dsID,
        ...
        return makeAssignment(DeploymentManagement.deploymentRequest(controllerId, dsID)
                ...
                .setConfirmationRequired(true).setUserAcceptanceRequired(MgmtRolloutUserAcceptanceRequired.NO).build());
    }
    ...
    protected Action prepareFinishedUpdate() {
        return prepareFinishedUpdate(TestdataFactory.DEFAULT_CONTROLLER_ID, "", false, false);
    }
    ...
    protected Action prepareFinishedUpdate(final String controllerId, final String distributionSet,
        ...
        final DistributionSet ds = testdataFactory.createDistributionSet(distributionSet, isRequiredMigrationStep, isSoftwareDowngradeEnabled);
        Target savedTarget = testdataFactory.createTarget(controllerId);
        savedTarget = getFirstAssignedTarget(
                assignDistributionSet(ds.getId(), savedTarget.getControllerId(), MgmtRolloutUserAcceptanceRequired.YES));
        ...
        savedAction = controllerManagement.addUpdateActionStatus(
                entityFactory.actionStatus().create(savedAction.getId()).status(DeviceActionStatus.RUNNING), null);

        return controllerManagement.addUpdateActionStatus(
                entityFactory.actionStatus().create(savedAction.getId()).status(DeviceActionStatus.FINISHED_FAILURE), null);
    }
    ...
    public void beforeAll() throws Exception {
        ...
        osType = WithSpringAuthorityRule.runAsPrivileged(() -> softwareModuleTypeManagement
                .update(entityFactory.softwareModuleType().update(osType.getId()).description(description).maxAssignments(1)));


        format = WithSpringAuthorityRule
                .runAsPrivileged(() -> testdataFactory.findOrCreateSoftwareModuleFormat(TestdataFactory.SM_FORMAT_APP));
        format = WithSpringAuthorityRule.runAsPrivileged(() -> softwareModuleFormatManagement
                .update(entityFactory.softwareModuleFormat().update(format.getId()).description(description)));
        ...
        appType = WithSpringAuthorityRule.runAsPrivileged(() -> softwareModuleTypeManagement
                .update(entityFactory.softwareModuleType().update(appType.getId()).description(description).maxAssignments(1)));
        ...
        runtimeType = WithSpringAuthorityRule.runAsPrivileged(() -> softwareModuleTypeManagement
                .update(entityFactory.softwareModuleType().update(runtimeType.getId()).description(description).maxAssignments(1)));

        swInstallerType = WithSpringAuthorityRule
                .runAsPrivileged(() -> testdataFactory.findSoftwareInstallerTypeByName(TestdataFactory.SM_INSTALLER_TYPE_0));
    ...
    private static String createTempDir() {
        try {
            return Files.createTempDirectory("AbstractIntegrationTests").toString() + "/" + RandomStringUtils.randomAlphanumeric(20);//NOSONAR
