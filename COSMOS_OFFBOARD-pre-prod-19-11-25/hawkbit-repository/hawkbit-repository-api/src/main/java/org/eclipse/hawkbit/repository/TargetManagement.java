...
public interface TargetManagement {
     ...
     * @throws EntityNotFoundException if given tagId or at least one of the targets do not exist
     */
     ...
     *
     * @param filterParams the filters to apply; only filters are enabled that have
    ...
    /**
     * @param controllerId  the unique identifier of the target (controller ID) whose tenant is to be updated
     * @param currentTenant the existing tenant of the target, used as a condition to verify the update
     * @param newTenant     the new tenant to be assigned to the target
     * @return {@code true} if the tenant was successfully updated; {@code false} if no match was found or no rows were updated
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_UPDATE_TARGET)
    boolean updateTenant(String controllerId, String currentTenant, String newTenant);
    ...
    /**
     * Count all targets for given {@link TargetFilterQuery} in given controllerIds and
     * that are compatible with the passed {@link DistributionSetType}.
     *
     * @param rsqlParam     filter definition in RSQL syntax
     * @param dsTypeId      ID of the {@link DistributionSetType} the targets need to be
     *                      compatible with
     * @param controllerIds controllerIds in the provided list
     * @return the found number of{@link Target}s
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    long countByRsqlAndInControllerIdAndCompatible(@NotEmpty String rsqlParam, @NotNull List<String> controllerIds, @NotNull Long dsTypeId);
     ...
     *
     * @param targetIDs the IDs of the targets to be deleted
     ...
     * @param rsqlParam           filter definition in RSQL syntax
     * @param distributionSetType type of the {@link DistributionSet} the targets must be
     *                            compatible with
     * @return a page of the found {@link Target}s
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    ...
    List<Target> findByTargetFilterAndTargetIdIn(String targetFilterQuery, List<String> targetId);
    ...
    /**
     * Find a {@link Target} based a given vin.
     *
     * @param vin to look for.
     * @return {@link Target}
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    Optional<Target> getByVin(@NotEmpty String vin);

    /**
     * Checks if the provided list of controller ids are present in cosmos and returns
     * list of registered - controllerIds present in cosmos
     * list of unregistered - controllerIds not present in cosmos
     * <p>
     * Note: The keys in the map are 'registered', 'unregistered' to retrieve respective lists.
     *
     * @param controllerIds list of controller ids to validate
     * @return This returns a map of ListOf(registered controller ids)and ListOf(unregistered controller ids)
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    Map<String, List<String>> separateRegisteredAndUnregisteredControllerIds(final Collection<String> controllerIds);
    ...
    /**
     * Find a {@link Target} based a given controllerId or Name or Serial Number.
     *
     * @param controllerId the controllerId of target
     * @param name         the name of target
     * @param serialNumber the serial number of target
     * @return Target
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    List<Target> findByControllerIdOrNameOrTarget(@NotEmpty String controllerId, @NotEmpty String name,
     ...
     * @param orderByDistributionId {@link DistributionSet#getId()} to be ordered by
    ...
    /**
     * Get controller software attributes attributes of given {@link Target}.
     *
     * @param controllerId of the target
     * @return controller software attributes  attributes as key/value pairs
     * @throws EntityNotFoundException if target with given ID does not exist
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    Map<String, String> getControllerSoftwareAttributes(@NotEmpty String controllerId);
     ...
     *
     * @param controllerId {@link Target} controller id the metadata has to be created
     *                     for
     * @param metadata     the meta data entries to create or update
     * @return the updated or created target metadata entries
     * @throws EntityNotFoundException          if given target does not exist
     * @throws EntityAlreadyExistsException     in case one of the metadata entry already exists for the
     *                                          specific key
     ...
     *
     * @param controllerId {@link Target} controller id of the metadata entry to be
     *                     updated
     * @param metadata     meta data entry to be updated
     * @return the updated meta data entry
     * @throws EntityNotFoundException in case the metadata entry does not exist and cannot be
     *                                 updated
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_UPDATE_REPOSITORY)
    ...
    /**
     * Finds list of targets for a given vehicle model id
     *
     * @param vehicleModelId {@link org.eclipse.hawkbit.repository.model.Vehicle} model id associated with targets
     * @return the List of targets
     * @throws EntityNotFoundException if the targets does not exist
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    List<Target> findByVehicleModelId(@NotNull Long vehicleModelId);

    /**
     * Finds list of targets for a given list of controller IDs.
     *
     * <p>This method retrieves a list of {@link Target}s based on the provided list of
     * controller IDs. It is assumed that the controller IDs are unique and
     * correspond to existing targets in the repository.
     *
     * @param controllerIds List of controller IDs associated with targets
     * @return the List of targets corresponding to the provided controller IDs
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    List<Target> findByControllerIdIn(List<String> controllerIds);

    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    List<Long> findTargetIdByControllerIds(@Param("controllerId") List<String> controllerId);
     ...
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    boolean isEcuNodeAddressMatchControllerIds(String ecuNodeAddress, List<String> controllerIds);

    /**
     * Retrieves a paginated list of inventory actions for a given target, sorted in descending order.
     *
     * @param controllerId the unique identifier of the target (controller ID) whose inventory actions are to be retrieved
     * @param pageable     the pagination and sorting parameters
     * @return a paginated list of {@link InventoryWithAction} objects
     * @throws EntityNotFoundException if the target with the given controller ID does not exist
     */
    Page<InventoryWithAction> findByTargetInventoryInDesc(@NotEmpty String controllerId, @NotNull Pageable pageable);

    /**
     * Counts the number of targets associated with the given controller ID.
     *
     * @param controllerId the unique identifier of the target (controller ID)
     * @return the count of targets associated with the given controller ID
     */
    long countByTargetControllerId(@Param("controllerId") String controllerId);
