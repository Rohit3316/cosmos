...
public interface DeploymentManagement {
     ...
     * add a {@link DeviceActionStatus#CANCELED} status to the action. However, it might be
    ...
    /**
     * @return the total amount of stored action from sp_action table
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    ...
    /**
     * Creates actions for a given rollout group within a rollout.
     *
     * @param rollout the rollout to which the group belongs
     * @param group the rollout group for which actions are to be created
     * @return the number of actions created for the rollout group
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    long createActionsForRolloutGroup(Rollout rollout, RolloutGroup group);
     ...
     * @param actionId
     *            the current ID of the actions
     * @return the found list of {@link Action}
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    Optional<Action> findByTargetIdAndActionIdAndActive(@NotEmpty String controllerId, @NotNull Long actionId, boolean active);
     ...
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    long countActionsByDistributionSetIdAndActiveIsTrueAndStatusIsNot(Long distributionSet, DeviceActionStatus status);
    ...
    /**
     * Retrieves the list of action status timestamps for a specific action and controller.
     * <p>
     * This method returns a list of status changes (with their timestamps) for the given action
     * and controller (target device). The status timestamps provide a history of the action's
     * progress and state transitions on the device.
     * </p>
     *
     * @param actionId the ID of the action to retrieve status timestamps for (must not be null)
     * @param controllerId the ID of the controller (target device) (must not be empty)
     * @return a list of {@link DeviceActionStatusTimestampResponse} containing status timestamps for the action
     *         and controller, or an empty list if none are found
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    List<DeviceActionStatusTimestampResponse> getActionStatusTimestamps(@NotNull Long actionId, @NotEmpty String controllerId);
     ...
     * @param actionStatusId
     *            the id of {@link ActionStatus} to retrieve the messages from
     * @return a page of messages with Error Code if action Status Failed by a specific {@link ActionStatus} id
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_TARGET)
    Page<String> findMessagesWithErrorCodeByActionStatusId(@NotNull Pageable pageable, long actionStatusId);
     ...
     * {@link Target}s to {@link DeviceActionStatus#CANCELED}
     *
     * @param targetIds
    ...
    /**
     * Starts all queued actions of an RolloutGroup parent.
     *
     * @param rolloutId
    ...
    int deleteActionsByStatusAndLastModifiedBefore(@NotNull Set<DeviceActionStatus> status, long lastModified);

    /**
     * Checks if there is an action for the device with the given controller ID
     * that is in the {@link DeviceActionStatus#CANCELLING} state.
     *
     * @param controllerId
     *            of target
