...
public interface DistributionSetManagement
    ...
    /**
     * Given a valid distribution and a target version update the version of the distribution.
     *
     * @param dsId
     * @param targetVersion
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_UPDATE_REPOSITORY)
    void updateDsTargetVersion(Long dsId, String targetVersion);


    /**
     * Assigns {@link SoftwareModule} to existing {@link DistributionSet}.
     *
     * @param setId    to assign and update
     * @param moduleId to get assigned
     ...
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_UPDATE_REPOSITORY)
    DistributionSet assignSoftwareModules(long setId, @NotEmpty Collection<DistributionSoftwareVersionWrapper> dsWrapper);

    /**
     * Assign a {@link DistributionSetTag} assignment to given
     * {@link DistributionSet}s.
     *
     * @param setIds to assign for
     ...
     *
     * @param setId    if the {@link DistributionSet} the metadata has to be created
     *                 for
     ...
     *
     * @param setId where meta data has to be deleted
     ...
     * @param setId    the distribution set id to retrieve the meta data from
     ...
     *
     * @param pageable the page request to page the result
     * @param setId    the distribution set id to retrieve the meta data count from
     ...
     *
     * @param setId of the {@link DistributionSet}
     * @param key   of the meta data element
     ...
     *
     * @param setId    to get unassigned form
     ...
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_UPDATE_REPOSITORY)
    DistributionSet unassignSoftwareModule(long setId, long moduleId, long version);

    /**
     * Unassign a {@link DistributionSetTag} assignment to given
     * {@link DistributionSet}.
     *
     * @param setId to unassign for
     ...
     *
     * @param setId    {@link DistributionSet} of the meta data entry to be updated
     ...
     *
     * @param set the ID of the {@link DistributionSet} to be set to invalid
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_UPDATE_REPOSITORY)
    void invalidate(DistributionSet set);

    /**
     * Get the list of {@link IDistributionSetModule} for the given {@link DistributionSet#getId()}.
     *
     * @param id the ID of the {@link DistributionSet}.
     * @return IDistributionSetModule list
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_REPOSITORY)
    List<IDistributionSetModule> getDistributionSetModule(final long id);

    /**
     * Get a list of expiring artifacts for a distribution set, before a given date.
     *
     * @param id      - distribution set id
     * @param endDate - Date, to find all the expiring artifacts before it.
     * @return - a list of all {@link ArtifactsExpiryDTO} for a distribution set before a given date.
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_REPOSITORY)
    List<ArtifactsExpiryDTO> getExpiringArtifactsForGivenDistributionSetBeforeEndDate(final long id, final long endDate);

    /**
     * Retrieves a list of {@link IDistributionSetModule} associated with the specified {@link DistributionSet}.
     * The method finds all modules linked to a specific distribution set ID and a target software version ID.
     *
     * @param dsSetId         the ID of the {@link DistributionSet} to query
     * @param targetVersionId the ID of the target software version
     * @return a list of {@link IDistributionSetModule} matching the specified distribution set ID and target version ID
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_REPOSITORY)
    List<IDistributionSetModule> findDsModuleBySoftwareModuleIdAndTargetVersionId(final long smId, final long targetVersionId);

    /**
     * Retrieves a single {@link IDistributionSetModule} associated with the specified {@link DistributionSet}.
     * This method looks up a module using the distribution set ID, software module ID, and target version ID.
     *
     * @param dsId            the ID of the {@link DistributionSet} to query
     * @param smId            the ID of the software module to query
     * @param targetVersionId the ID of the target software version
     * @return the {@link IDistributionSetModule} matching the specified criteria, or {@code null} if none found
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_READ_REPOSITORY)
    IDistributionSetModule findByDsIdAndSoftwareModuleIdAndTargetVersionId(final long dsId, final long smId, final long targetVersionId);


    /**
     * Creates a new {@link IDistributionSetModule} and persists it in the repository.
     * This method is used to associate a distribution set module with the appropriate distribution set.
     *
     * @param create the {@link IDistributionSetModule} to be created
     * @return the newly created {@link IDistributionSetModule}
     */
    @PreAuthorize(SpringEvalExpressions.HAS_AUTH_CREATE_REPOSITORY)
    IDistributionSetModule createDistributionSetModule(final IDistributionSetModule create);



    /**
     * Fetches module IDs and software version IDs for the given distrubution ID.
     *
     ...
     */
    List<IDistributionSetModule> getModulesAndVersionsByRolloutId(Long rolloutId);
