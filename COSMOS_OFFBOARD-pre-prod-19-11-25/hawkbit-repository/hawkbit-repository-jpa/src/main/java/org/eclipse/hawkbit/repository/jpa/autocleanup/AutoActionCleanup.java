 ...
 * actions which are in a certain {@link DeviceActionStatus}. It is recommended to
...
public class AutoActionCleanup implements CleanupTask {
    ...
    private static final long ACTION_CLEANUP_ACTION_EXPIRY_DEFAULT = TimeUnit.DAYS.toSeconds(30);
    private static final EnumSet<DeviceActionStatus> EMPTY_STATUS_SET = EnumSet.noneOf(DeviceActionStatus.class);
    ...
    public void run() {
        ...
        final EnumSet<DeviceActionStatus> status = getActionStatus();
        if (!status.isEmpty()) {
            final long lastModified = Instant.now().getEpochSecond() - getExpiry();
            ...
            LOGGER.debug("Deleted {} actions in status {} which have not been modified since {} ({})", actionsCount,
                    status, Instant.ofEpochSecond(lastModified), lastModified);
        }
    }
    ...
    private EnumSet<DeviceActionStatus> getActionStatus() {
        final TenantConfigurationValue<String> statusStr = getConfigValue(ACTION_CLEANUP_ACTION_STATUS, String.class);
        if (statusStr != null) {
            return Arrays.stream(statusStr.getValue().split("[;,]")).map(DeviceActionStatus::valueOf)
                    .collect(Collectors.toCollection(() -> EnumSet.noneOf(DeviceActionStatus.class)));
        }
