...
public interface DistributionSetRepository
     ...
     */
    @Query("SELECT COUNT(ds) FROM JpaDistributionSet ds JOIN ds.modules dsModule JOIN dsModule.sm sm WHERE sm.id = :moduleId")
    ...
    /**
     * Finds {@link DistributionSet}s where given {@link SoftwareModule}
     *
     * @param moduleId to search for
     * @return {@link List} of found {@link DistributionSet}s
     */
    @Query("SELECT ds FROM JpaDistributionSet ds JOIN ds.modules dsModule JOIN dsModule.sm sm WHERE sm.id = :moduleId")
    List<DistributionSet> getDistributionSetByModulesId(Long moduleId);

    /**
     * Retrieves an {@link Optional} containing a {@link DistributionSet} that is associated with both the specified
     * software module ID and distribution set ID.
     *
     * This query ensures that the software module, identified by the given {@code moduleId}, is already linked
     * to the distribution set, identified by the given {@code dsId}. If no such link exists, an empty {@link Optional}
     * will be returned.
     *
     * @param moduleId the ID of the software module to check for linkage
     * @param dsId the ID of the distribution set to check for linkage
     * @return an {@link Optional} containing the {@link DistributionSet} if the module is linked, otherwise an empty {@link Optional}
     */
    @Query("SELECT ds FROM JpaDistributionSet ds JOIN ds.modules dsModule JOIN dsModule.sm sm WHERE sm.id = :moduleId and ds.id = :dsId")
    Optional<DistributionSet> getDistributionSetByModulesIdAndDsId(Long moduleId, Long dsId);
}
