...
@Table(name = "sp_target", indexes = {
        ...
        "controller_id", "serial_number"}, name = "uk_serial_number_controller_id"))
...
public class JpaTarget extends AbstractJpaNamedEntity implements Target, EventAwareEntity {
    ...
    @Column(name = "serial_number", length = Target.SERIAL_NUMBER_MAX_SIZE, updatable = false, nullable = false)
    @Size(min = 1, max = Target.SERIAL_NUMBER_MAX_SIZE)
    @NotNull
    @Pattern(regexp = "[\\S]*", message = "has whitespaces which are not allowed")
    ...
    @CascadeOnDelete
    @OneToMany(mappedBy = "target", fetch = FetchType.LAZY, targetEntity = JpaTargetSoftware.class)
    private List<TargetSoftware> softwares;
    ...
    //@ManyToOne(fetch = FetchType.LAZY, targetEntity = JpaVehicle.class, optional = false)
    //@JoinColumn(name = "vehicle_model_id", nullable = false, updatable = false, foreignKey = @ForeignKey(value = ConstraintMode.CONSTRAINT, name = "fk_vehicle_model_id"))
    @Column(name = "vehicle_model_id")
    @NotNull
    private Long vehicleModelId;

    @Column(name = "vin", length = Target.VIN_MAX_SIZE, updatable = false, nullable = false)
    @Size(min = 1, max = Target.VIN_MAX_SIZE)
    @NotNull
    @Pattern(regexp = "[\\S]*", message = "has whitespaces which are not allowed")
    private String vin;
     ...
     * @param name         name of the {@link Target}
     * @param serialNumber serialNumber of the {@link Target}
     * @param vin vin of the {@link Target}
     */
    public JpaTarget(final String controllerId, final String name, final String serialNumber, final Long vehicleModelId, final String vin) {
        this(controllerId, name, serialNumber, SecurityTokenGeneratorHolder.getInstance().generateToken(), vehicleModelId, vin);
    }
     ...
     * @param vin vin of the {@link Target}
     */
    public JpaTarget(final String controllerId, final String name, final String serialNumber, final String securityToken, final Long vehicleModelId, final String vin) {
    ...
    public static String truncateControllerIdToMaxNameLength(final String controllerId) {
    ...
    public PollStatus getPollStatus() {
        ...
        return SystemSecurityContextHolder.getInstance().getSystemSecurityContext().runAsSystem(() -> {
            ...
            final LocalDateTime lastPollDate = LocalDateTime.ofInstant(Instant.ofEpochSecond(lastTargetQuery),
                    ZoneId.of("UTC"));
     ...
     */
    public void setVehicleModelId(final Long vehicleModelId) {
        this.vehicleModelId = vehicleModelId;
    }
     ...
     */
    public List<TargetSoftware> getSoftwares() {
        if (softwares == null) {
            return Collections.emptyList();
        }
    ...
    public void setSoftwares(List<TargetSoftware> softwares) {
    ...
    public void fireDeleteEvent(final DescriptorEvent descriptorEvent) {
        EventPublisherHolder.getInstance().getEventPublisher()
                .publishEvent(new TargetDeletedEvent(getTenant(), getId(), getControllerId(),
