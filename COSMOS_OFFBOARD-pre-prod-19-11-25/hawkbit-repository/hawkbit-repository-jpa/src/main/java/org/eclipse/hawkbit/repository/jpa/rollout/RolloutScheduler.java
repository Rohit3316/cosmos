...
public class RolloutScheduler {
    ...
    private static final String PROP_END_SCHEDULER_CRON_EXPRESSION = "${hawkbit.rollout.end.scheduler.cron:0 0 0,12 * * *}"; // end scheduler cron expression, default cron is 12 AM/PM daily
    ...
    @Scheduled(cron = PROP_END_SCHEDULER_CRON_EXPRESSION, zone = "UTC")
    public void runningRolloutEndScheduler() {
        LOGGER.debug("rollout end schedule checker has been triggered.");
        // run this code in system code privileged to have the necessary
        // permission to query and create entities.
        systemSecurityContext.runAsSystem(() -> {
            LOGGER.debug("Starting to process tenants for rollout end.");
            systemManagement.forEachTenant(tenant -> {
                LOGGER.debug("Processing tenant: {}", tenant);
                rolloutHandler.handleEnd();
            });
            LOGGER.debug("Finished processing tenants for rollout end.");
            return null;
        });
    }
}
