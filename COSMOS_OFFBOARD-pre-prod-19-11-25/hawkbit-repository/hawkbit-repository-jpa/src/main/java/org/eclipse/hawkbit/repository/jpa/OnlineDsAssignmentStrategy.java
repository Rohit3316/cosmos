...
public class OnlineDsAssignmentStrategy extends AbstractDsAssignmentStrategy {
    ...
    public void setAssignedDistributionSetAndTargetStatus(final JpaDistributionSet set, final List<List<Long>> targetIds,
        ...
        targetIds.forEach(tIds -> targetRepository.setAssignedDistributionSetAndUpdateStatus(TargetUpdateStatus.PENDING,
                set, Instant.now().getEpochSecond(), currentUser, tIds));
    ...
    public JpaAction createTargetAction(final String initiatedBy, final TargetWithUserAcceptanceRequired targetWithUserAcceptanceRequired,
        ...
        final JpaAction result = super.createTargetAction(initiatedBy, targetWithUserAcceptanceRequired, targets, set);
        if (result != null) {
            final boolean confirmationRequired = targetWithUserAcceptanceRequired.isConfirmationRequired()
                    && result.getTarget().getAutoConfirmationStatus() == null;

            result.setStatus(DeviceActionStatus.RUNNING);
        }
        return result;
    }
    ...
    private static Stream<Action> filterCancellations(final List<Action> actions) {
        return actions.stream().filter(action -> {
            final DeviceActionStatus actionStatus = action.getStatus();
            return DeviceActionStatus.CANCELING != actionStatus && DeviceActionStatus.CANCELED != actionStatus;
        });
    }
