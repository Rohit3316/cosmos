...
public class JpaTenantConfigurationManagement implements TenantConfigurationManagement {
     ...
     * @param tenant               tenant for which configurations has to be fetched for
    ...
    @Override
    @Cacheable(value = "tenantConfiguration", key = "#configurationKeyName")
    public <T extends Serializable> TenantConfigurationValue<T> getConfigurationValueByTenant(final String configurationKeyName,
                                                                                              final Class<T> propertyType, final String tenant) {

        final TenantConfigurationKey configurationKey = tenantConfigurationProperties.fromKeyName(configurationKeyName);

        validateTenantConfigurationDataType(configurationKey, propertyType);

        final TenantConfiguration tenantConfiguration = systemSecurityContext.runAsTenantAsLoggedUser(
                () -> DeploymentHelper.runInNewTransaction(txManager, "fetch-tenant-config", status -> tenantConfigurationRepository.findByKey(configurationKey.getKeyName())), tenant);
    ...
    public <T extends Serializable> TenantConfigurationValue<T> getConfigurationValue(
            final String configurationKeyName, final String tenant) {
        //change this to check for tenant as well in the repo
        final TenantConfigurationKey configurationKey = tenantConfigurationProperties.fromKeyName(configurationKeyName);

        return getConfigurationValueByTenant(configurationKeyName, configurationKey.getDataType(), tenant);
    }
    ...
    public <T extends Serializable> TenantConfigurationValue<T> addOrUpdateConfiguration(
        ...
        // Vehicle Log Level validation
        if (TenantConfigurationProperties.TenantConfigurationKey.VEHICLE_LOG_LEVEL.equals(configurationKeyName)) {
            try {
                int logLevel = Integer.parseInt(value.toString());
                if (logLevel < 0 || logLevel > 7) {
                    throw new TenantConfigurationValidatorException("Invalid Vehicle Log Level");
                }
            } catch (NumberFormatException e) {
                throw new TenantConfigurationValidatorException("Invalid Vehicle Log Level - must be numeric", e);
            }
        }
        ...
        // Check if the value is a List, convert it to string, otherwise, use toString for non-list values
        String keyValue = value instanceof List<?> ? convertListToString((List<?>) value) : value.toString();

        if (tenantConfiguration == null) {
            tenantConfiguration = new JpaTenantConfiguration(configurationKey.getKeyName(), keyValue);
        } else {
            tenantConfiguration.setValue(keyValue);
        }
        ...
        @SuppressWarnings("unchecked") final Class<T> clazzT = (Class<T>) value.getClass();
    ...
    /**
     * Converts a list of any type to a comma-separated string.
     *
     * @param values The list to convert.
     * @param <T> The type of elements in the list.
     * @return A comma-separated string representation of the list.
     */
    private static <T> String convertListToString(List<T> values) {
        try {
    ...
    /**
     * fetches default value from config for the given configurationKeyName and
     * then add or updates configuration for a specific tenant.
     *
     * @param configurationKeyName the key of the configuration     *
     **/
    @Override
    ...
    @Retryable(include = {
            ConcurrencyFailureException.class}, maxAttempts = Constants.TX_RT_MAX, backoff = @Backoff(delay = Constants.TX_RT_DELAY))
    public <T extends Serializable> TenantConfigurationValue<T> addOrUpdateConfiguration(final String configurationKeyName) {
        final TenantConfigurationKey configurationKey = tenantConfigurationProperties.fromKeyName(configurationKeyName);
        return addOrUpdateConfiguration(configurationKey.getKeyName(),
                Objects.requireNonNull(conversionService.convert(configurationKey.getDefaultValue(), configurationKey.getDataType())));
    }
    ...
    @Override
    public Boolean existsByKey(final String configurationKeyName) {
        return tenantConfigurationRepository.existsByKey(configurationKeyName);
    }

    /**
     * Get array of CDN for tenant configuration
     *
     * @return String[] thw array of CDN
     */
    @Override
    public String[] getCdn() {
        return tenantConfigurationProperties.getCdn();
    }
}
