...
public class JpaTargetTagManagement implements TargetTagManagement {
    ...
    public JpaTargetTagManagement(final TargetTagRepository targetTagRepository,
            ...
            final Database database, TargetManagement targetManagement, EntityManager entityManager) {
    ...
    @Retryable(include = {
            ConcurrencyFailureException.class}, maxAttempts = Constants.TX_RT_MAX, backoff = @Backoff(delay = Constants.TX_RT_DELAY))
    public TargetTag create(final TargetTag targetTag) {
        return targetTagRepository.save((JpaTargetTag) targetTag);
    }

    @Override
    @Transactional
    public TargetTag createTargetTag(String tagName, String description, String colour) {
        TargetTag tag = targetTagRepository.save(new JpaTargetTag(tagName, description, colour));
        entityManager.flush();
        return tag;
    }
    ...
    @Override
    @Transactional
    public void associateTagWithTargets(TargetTag tag, List<String> controllerIds) {
        targetManagement.assignTag(controllerIds, tag.getId());
    }
