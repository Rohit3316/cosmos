...
public final class QuotaHelper {
    ...
    /**
     * Assert that the number of assignments in a request does not exceed the
     * limit and return the error message given, if failed.
     *
     * @param requested
     *            the number of assignments that are to be made
     * @param limit
     *            the maximum number of assignments per request
     * @param message
     *            the error message on assignment failure.
     */
    public static void assertAssignmentQuotaWithMessage(final long requested, final long limit, final String message) {
        if (requested > limit) {
            LOG.warn(message);
            throw new AssignmentQuotaExceededException(message);
        }
