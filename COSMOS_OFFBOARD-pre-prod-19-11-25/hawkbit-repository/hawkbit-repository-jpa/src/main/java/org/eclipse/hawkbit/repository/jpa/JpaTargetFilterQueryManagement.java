...
public class JpaTargetFilterQueryManagement implements TargetFilterQueryManagement {
    ...
    @Retryable(include = {
            ConcurrencyFailureException.class }, maxAttempts = Constants.TX_RT_MAX, backoff = @Backoff(delay = Constants.TX_RT_DELAY))
    public TargetFilterQuery create(final TargetFilterQuery targetFilterQuery) {
        JpaTargetFilterQuery filterQuery = (JpaTargetFilterQuery) targetFilterQuery;

        if (filterQuery.getQuery() != null) {
            // validate the RSQL query syntax
            RSQLUtility.validateRsqlFor(filterQuery.getQuery(), TargetFields.class);

            // enforce the 'max targets per auto assign' quota right here even
            // if the result of the filter query can vary over time
            if (filterQuery.getAutoAssignDistributionSet() != null) {
                Long dsId = filterQuery.getAutoAssignDistributionSet().getId();
                WeightValidationHelper.usingContext(systemSecurityContext, tenantConfigurationManagement).validate(filterQuery);
                assertMaxTargetsQuota(filterQuery.getQuery(), filterQuery.getName(), dsId);
            }
        }

        return targetFilterQueryRepository.save(filterQuery);
    }
    ...
    public Slice<TargetFilterQuery> findByName(final Pageable pageable, final String name) {
        ...
        return targetFilterQueryRepository.findByNameContainingIgnoreCase(name, pageable);
    }
    ...
    public long countByName(final String name) {
        ...
        return targetFilterQueryRepository.countByNameContainingIgnoreCase(name);
    }

    @Override
    public Page<TargetFilterQuery> findByRsql(final Pageable pageable, final String rsqlFilter) {
        final List<Specification<JpaTargetFilterQuery>> specList = StringUtils.hasText(rsqlFilter)
    ...
    public Slice<TargetFilterQuery> findByQuery(final Pageable pageable, final String query) {
        final List<Specification<JpaTargetFilterQuery>> specList = StringUtils.hasText(query)
    ...
    public Page<TargetFilterQuery> findByAutoAssignDSAndRsql(final Pageable pageable, final long setId,
        ...
        if (StringUtils.hasText(rsqlFilter)) {
    ...
    public TargetFilterQuery updateAutoAssignDS(final AutoAssignDistributionSetUpdate update) {
        ...
        if (update.getDsId() == null) {
            ...
            targetFilterQuery.setAutoAssignUserAcceptanceRequired(null);
            ...
            targetFilterQuery.setAutoAssignUserAcceptanceRequired(sanitizeAutoAssignUserAcceptanceRequired(update.getUserAcceptanceRequired()));
    ...
    private static MgmtRolloutUserAcceptanceRequired sanitizeAutoAssignUserAcceptanceRequired(final MgmtRolloutUserAcceptanceRequired userAcceptanceRequired) {
    ...
    public void cancelAutoAssignmentForDistributionSet(final long setId) {
        targetFilterQueryRepository.unsetAutoAssignDistributionSetAndUserAcceptanceRequired(setId);
