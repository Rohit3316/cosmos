...
public interface TenantConfigurationRepository extends BaseEntityRepository<JpaTenantConfiguration, Long> {
    ...
    /**
     * Checks a tenant configuration by tenant and key.
     *
     * @param keyName the name of the key to be verified to be present in the tenant config
     */
    Boolean existsByKey(String keyName);
    ...
    @Transactional
    @Query("SELECT t FROM JpaTenantConfiguration t WHERE t.tenant = :tenant")
    List<JpaTenantConfiguration> getByTenant(@Param("tenant") String tenant);
