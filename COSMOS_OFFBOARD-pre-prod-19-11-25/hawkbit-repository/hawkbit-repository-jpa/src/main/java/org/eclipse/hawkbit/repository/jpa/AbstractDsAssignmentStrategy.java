...
public abstract class AbstractDsAssignmentStrategy {
    ...
    protected List<Long> overrideObsoleteUpdateActions(final Collection<Long> targetsIds) {
        ...
        final List<JpaAction> activeActions = actionRepository
                .findByActiveAndTargetIdInAndActionStatusNotEqualToAndDistributionSetNotRequiredMigrationStep(
                        targetsIds, DeviceActionStatus.CANCELING);
    ...
    protected List<Long> closeObsoleteUpdateActions(final Collection<Long> targetsIds) {
        ...
        return activeActions.stream().map(action -> {
            action.setStatus(DeviceActionStatus.CANCELED);
    ...
    public JpaAction createTargetAction(final String initiatedBy, final TargetWithUserAcceptanceRequired targetWithUserAcceptanceRequired,
            final List<JpaTarget> targets, final JpaDistributionSet set) {
        final Optional<JpaTarget> optTarget = targets.stream()
                .filter(t -> t.getControllerId().equals(targetWithUserAcceptanceRequired.getControllerId())).findFirst();

        // create the action
        return optTarget.map(target -> {
            assertActionsPerTargetQuota(target, 1);
            final JpaAction actionForTarget = new JpaAction();
            actionForTarget.setUserAcceptanceRequired(targetWithUserAcceptanceRequired.getUserAcceptanceRequired());
            actionForTarget.setForcedTime(targetWithUserAcceptanceRequired.getForceTime());
            actionForTarget.setWeight(targetWithUserAcceptanceRequired.getWeight());
            actionForTarget.setActive(true);
            actionForTarget.setTarget(target);
            actionForTarget.setDistributionSet(set);
            actionForTarget.setMaintenanceWindowSchedule(targetWithUserAcceptanceRequired.getMaintenanceSchedule());
            actionForTarget.setMaintenanceWindowDuration(targetWithUserAcceptanceRequired.getMaintenanceWindowDuration());
            actionForTarget.setMaintenanceWindowTimeZone(targetWithUserAcceptanceRequired.getMaintenanceWindowTimeZone());
            actionForTarget.setInitiatedBy(initiatedBy);
            return actionForTarget;
        }).orElseGet(() -> {
            LOG.warn("Cannot find target for targetWithUserAcceptanceRequired '{}'.", targetWithUserAcceptanceRequired.getControllerId());
            return null;
        });
    }
    ...
    private void assertActionsPerTargetQuota(final Target target, final int requested) {
        ...
        QuotaHelper.assertAssignmentQuota(target.getId(), requested, quota, Action.class, Target.class,
                (id) -> actionRepository.countByTargetIdAndActive(id, true));
    }
