...
public final class WeightValidationHelper {
    ...
    public void validate(final List<DeploymentRequest> deploymentRequests) {
        final long assignmentsWithWeight = deploymentRequests.stream()
                .filter(request -> request.getTargetWithUserAcceptanceRequired().getWeight() != null).count();
     ...
     *            the target filter query
     */
    public void validate(final JpaTargetFilterQuery targetFilterQuery) {
        validateWeight(targetFilterQuery.getAutoAssignWeight().orElse(null));
