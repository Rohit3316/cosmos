...
public class JpaActionStatusCreate extends AbstractActionStatusCreate<ActionStatusCreate>
    ...
    public JpaActionStatus build() {
        final JpaActionStatus result = new JpaActionStatus(status, getOccurredAt().orElse(Instant.now().getEpochSecond()));
        if (messages != null) {
        ...
        if(userAcceptanceMessageJob1 != null) {
            result.setUserAcceptanceMessageJob1(userAcceptanceMessageJob1);
        }
        return result;
    }
}
