...
class DefaultDownloadIdCacheTest {
    ...
    private static final  String KNOWN_KEY = "12345";
    ...
    void putKeyAndValueIsDelegatedToCacheManager() {
        final DownloadArtifactCache value = new DownloadArtifactCache(DownloadType.BY_SHA1, KNOWN_KEY);

        underTest.put(KNOWN_KEY, value);

        verify(cacheMock).put(cacheManagerKeyCaptor.capture(), cacheManagerValueCaptor.capture());

        assertThat(cacheManagerKeyCaptor.getValue()).isEqualTo(KNOWN_KEY);
    ...
    void getValueReturnsTheAssociatedValueForKey() {
        ...
        final DownloadArtifactCache knownValue = new DownloadArtifactCache(DownloadType.BY_SHA1, KNOWN_KEY);

        when(cacheMock.get(KNOWN_KEY)).thenReturn(new SimpleValueWrapper(knownValue));

        final DownloadArtifactCache downloadArtifactCache = underTest.get(KNOWN_KEY);
    ...
    void getValueReturnsNullIfNoKeyIsAssociated() {
        ...
        when(cacheMock.get(KNOWN_KEY)).thenReturn(new SimpleValueWrapper(null));
