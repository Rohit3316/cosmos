...
public interface MgmtActionRestApi {
    ...
    @GetMapping(value = MgmtRestConstants.ACTION_V1_REQUEST_MAPPING_TENANT, produces = {MediaTypes.HAL_JSON_VALUE,
    ...
    @GetMapping(value = MgmtRestConstants.ACTION_SINGLE_V1_REQUEST_MAPPING_TENANT, produces = {
            MediaTypes.HAL_JSON_VALUE, MediaType.APPLICATION_JSON_VALUE})
    @Deprecated
    ResponseEntity<MgmtAction> getAction(@PathVariable("actionId") Long actionId, @PathVariable("tenantId") Long tenantId);

    /**
     * Handles the GET request of retrieving {@link DeploymentLog}s for given {@link Action} and {@link Target}
     *
     * @param controllerId The ID of the requested target
     * @param actionId The ID of the requested action
     * @return PagedList the {@link MgmtDeploymentLog}s
     */
    @Operation(summary = "Return action by id", description = "Handles the GET request of retrieving a single action by actionId.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved"),
    ...
    @GetMapping(value = MgmtRestConstants.ACTION_DEPLOYMENT_LOGS_MAPPING_TENANT, produces = {
            MediaTypes.HAL_JSON_VALUE, MediaType.APPLICATION_JSON_VALUE})
    ResponseEntity<PagedList<MgmtDeploymentLog>> getActionsDeploymentLogs(@PathVariable("controllerId") String controllerId, @PathVariable("actionId") Long actionId, @PathVariable("tenantId") Long tenantId);
