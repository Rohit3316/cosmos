...
public final class MgmtSoftwareModuleTypeMapper {
    ...
    public static List<MgmtSoftwareModuleType> toTypesResponse(Long tenantId, final Collection<SoftwareModuleType> types) {
        if (types == null) {
            return Collections.emptyList();
        }

        return new ResponseList<>(
                types.stream().map(type -> MgmtSoftwareModuleTypeMapper.toResponse(tenantId, type)).collect(Collectors.toList()));
    }

    public static MgmtSoftwareModuleType toResponse(Long tenantId, final SoftwareModuleType type) {
        ...
        result.add(linkTo(methodOn(MgmtSoftwareModuleTypeRestApi.class).getSoftwareModuleType(tenantId, result.getModuleId()))
                .withSelfRel().expand());
